...
impl DbKind for MapEnvironment {
    fn validate(&self, _key: &Token, block: &Block, data: &Everything) {
        ...
        vd.field_numeric("sun_intensity");
        vd.field_list_numeric_exactly("sun_direction", 3);
        vd.field_list_numeric_exactly("shadow_direction_offset", 3);
        ...
        vd.field_numeric("fog_begin");
        vd.field_numeric("fog_end");
        vd.field_numeric("fog_max");

        vd.field_list_numeric_exactly("water_sun_direction_offset", 3);

        vd.field_numeric("hue_offset");
        vd.field_numeric("saturation_scale");
        vd.field_numeric("value_scale");
        vd.field_list_numeric_exactly("colorbalance", 3);
        vd.field_validated_block("levels_min", validate_camera_color);
        vd.field_validated_block("levels_max", validate_camera_color);

        vd.field_numeric("bloom_width");
        vd.field_numeric("bloom_scale");
        vd.field_numeric("bright_threshold");

        vd.field_numeric("hdr_min_adjustment");
        vd.field_numeric("hdr_max_adjustment");
        vd.field_numeric("hdr_adjustment_speed");
        vd.field_numeric("tonemap_middlegrey");
        vd.field_numeric("tonemap_whiteluminance");

        vd.field_value("exposure_function"); // TODO
        vd.field_numeric("exposure");
...
fn validate_curve(block: &Block, data: &Everything) {
    let mut vd = Validator::new(block, data);
    vd.field_numeric("shoulder_strength");
    vd.field_numeric("linear_strength");
    vd.field_numeric("linear_angle");
    vd.field_numeric("toe_strength");
    vd.field_numeric("toe_numerator");
    vd.field_numeric("toe_denominator");
    vd.field_numeric("linear_white");
}

fn validate_dof(block: &Block, data: &Everything) {
    let mut vd = Validator::new(block, data);
    vd.field_bool("enabled");
    vd.field_numeric("dof_samplecount");
    vd.field_numeric("dof_baseradius");
    vd.field_numeric("dof_blurblendmin");
    vd.field_numeric("dof_blurblendmax");
    vd.field_numeric("dof_blurmin");
    vd.field_numeric("dof_blurmax");
    vd.field_numeric("dof_blurscale");
    vd.field_numeric("dof_blurexponent");
    vd.field_numeric("dof_heightmin");
    vd.field_numeric("dof_heightmax");
}
